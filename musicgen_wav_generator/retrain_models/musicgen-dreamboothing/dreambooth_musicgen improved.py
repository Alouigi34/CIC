...
# ──────────────────────────────────────────────────────────────────────────────
# DEV-MODE SHORTCUT
# If you run this file from an IDE / notebook, set `DEV_ARGS` below and the
# script will pretend they were typed on the command-line.  When you run it
# from a real shell, the normal argparse behaviour is untouched.
# ──────────────────────────────────────────────────────────────────────────────

# DEV_ARGS: list[str] | None = [
#     # ───────────── Model ─────────────
#     "--model_name_or_path", "facebook/musicgen-melody",
#     "--use_lora",                                   # ← train only LoRA adapters
#                                                     #   (tiny, fits in GPU RAM)

#     # ───────────── Dataset ───────────
#     "--dataset_name", "/home/alels_star/Desktop/AI_composition_assistant/"
#                       "v0.01/musicgen_wav_generator/retrain_models/"
#                       "musicgen-dreamboothing/punk_dataset",
...
#     "--num_train_epochs",           "15", #"USE 1 for produced output model same as base model"

#     # mini-batch fits in 15 GB :
#     "--per_device_train_batch_size","2",
#     "--gradient_accumulation_steps","8",   # 1×32 ≈ 32 effective batch
#     "--per_device_eval_batch_size", "1",

#     "--max_duration_in_seconds",    "30",    # 5-second clips ⇒ small sequence
#     "--min_duration_in_seconds",    "1",
#     #"--fp16",
#     "--gradient_checkpointing",

#     # optimiser / LR
#     "--learning_rate",              "2e-4",   #"USE 0.0 for produced output model same as base model",
#     "--adam_beta1",                 "0.9",
#     "--adam_beta2",                 "0.99",
#     "--weight_decay",               "0.1",

#     # generation & guidance (optional, but avoids key-errors later)
#     "--guidance_scale",             "3.0",
...
# Set to `None` when you want to fall back to real CLI / JSON.
# ──────────────────────────────────────────────────────────────────────────────
...
DEV_ARGS: list[str] | None = [
    ...
    # precision (prefer bf16 on Ampere+/newer; otherwise switch to --fp16)
    "--fp16",

    # generation & guidance (for eval/generate)
    "--guidance_scale",             "3.0",
    "--generation_max_length",      "800",   # ≈20s at ~40 tok/s
...
class DataSeq2SeqTrainingArguments:
    ...
    dataset_data_dir: str = field(
        default=None,
        metadata={
            "help": "Local folder that contains the dataset (for builders like audiofolder)."
        },
    )
...
def main():
    ...
    # --- three ways to feed arguments ------------------------------------------
    # 1. a single JSON path      → script.py my_config.json
    # 2. normal CLI parameters   → script.py --model_name ... --dataset_name ...
    # 3. *DEV-MODE* inside IDE   → DEV_ARGS list above
    # ---------------------------------------------------------------------------
    if len(sys.argv) == 2 and sys.argv[1].endswith(".json"):
        model_args, data_args, training_args = parser.parse_json_file(
            json_file=os.path.abspath(sys.argv[1])
        )
    elif DEV_ARGS is not None and (hasattr(sys, "ps1") or sys.argv[0].endswith("ipykernel_launcher.py")):
        # running interactively ⇒ pretend DEV_ARGS were passed on CLI
    ...
    # ── adjust special-token ids *before* we build the model ────────────────
    if model_args.pad_token_id is not None:
        config.pad_token_id = model_args.pad_token_id
        # decoder section might exist on some configs
        if hasattr(config, "decoder"):
            config.decoder.pad_token_id = model_args.pad_token_id
    
    if model_args.decoder_start_token_id is not None:
        config.decoder_start_token_id = model_args.decoder_start_token_id
        if hasattr(config, "decoder"):
            config.decoder.decoder_start_token_id = model_args.decoder_start_token_id
    # ────────────────────────────────────────────────────────────────────────
    ...
    if training_args.gradient_checkpointing and not model.is_gradient_checkpointing:
        # we only need checkpoints in the (huge) decoder,
        # so every other sub-module stays normal
    ...
    def prepare_audio_features(batch):
        ...
        # --- new robust loader -----------------------------------
        wav_path = batch[target_audio_column_name]["path"]
        ...
        # resample if the dataset SR differs from the model SR
        target_sr = audio_encoder_feature_extractor.sampling_rate
        if sr != target_sr:
            waveform = torchaudio.functional.resample(waveform, sr, target_sr)
        
        labels = audio_encoder_feature_extractor(
            waveform.mean(0).numpy(), sampling_rate=target_sr
        )
        batch["labels"] = labels["input_values"]
        batch["target_length"] = waveform.shape[-1]
        # ----------------------------------------------------------
    ...
    #eval_metrics = {}              # re-enable later for validation-only runs
