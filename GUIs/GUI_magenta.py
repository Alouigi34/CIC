...
def button(lbl, x, y, w, h, cb, col=BLUE):
    ...
    # center text vertically with a bit of padding
...
while True:
    ...
    # GENERATE panel
    draw("GENERATE", GEN_X, TRAIN_Y, BLUE)
    gen_labels = [
    ...
    for i, (lbl, cb) in enumerate([
        ...
        ("Edit Length",       lambda: start_edit("gen_steps")),
        ("Edit Temp",         lambda: start_edit("gen_temp")),
    ]):
        y = TRAIN_Y + (5 + i)*GEN_SPACING
        bx = GEN_X + BTN_X_OFFSET if i < 5 else GEN_X + BTN_X_OFFSET
        #bw = BTN_W if i < 5 else BTN_W//2
        bw = BTN_W if i < 5 else (BTN_W * 2 // 3)

        btns.append(
            button(lbl, bx, y, bw, BTN_H, cb)
        )
            ...
            if e.key == pygame.K_RETURN:
                try:
                    if editing_field in ("run_steps","batch_size","num_outputs","gen_steps"):
                        ...
                        val = float(text_input) if editing_field=="gen_temp" else text_input
