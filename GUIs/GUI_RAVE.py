...
COMMON      = BASE_DIR / "common_data_space"
...
def button(lbl, x, y, w, h, cb, col=COL_BTN):
    ...
    # draw text at original coords + padding
    screen.blit(font.render(lbl, True, COL_TEXT), (sc(x + 10), sc(y + (h - (font.get_height()/SCALE))//2)))
    return r, cb
...
input_wav    = str(COMMON / "input_data" / "RAVE" / "song2_cutted.wav")
default_out  = Path(input_wav).stem + "_out.wav"
output_wav   = str(COMMON / "generated_data" / "RAVE" / default_out)
duration     = 9000001.0
...
def set_input():
    ...
    if p:
        ...
        output_wav = str(COMMON / "generated_data" / "RAVE" / stem)
        message = "Input set"
...
while True:
    ...
    if editing_field:
        draw(f"Editing {editing_field}: {text_input}", 30, HEIGHT/SCALE - 80, (180,180,180))
