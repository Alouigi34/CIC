...
class Dashboard(tk.Tk):
    def __init__(self):
        ...
        # init mixer & metadata
        mixer.init()
        mixer.set_num_channels(2)  # for Play Both
        ...
        # playback & compare controls
        ctl = ttk.Frame(right); ctl.pack(pady=5)
        ttk.Button(ctl, text="Play",       command=self.play).pack(side="left", padx=5)
        ttk.Button(ctl, text="Play Both",  command=self.play_both).pack(side="left", padx=5)
        ...
        ttk.Button(ctl, text="Compare",    command=self.compare).pack(side="left", padx=5)
    ...
    def _view_single(self, sel):
        ...
        self.current_rel  = os.path.relpath(path, COMMON_DIR)
    ...
    def _show_audio(self, path):
        ...
        # waveform
        self.ax_wf.clear()
        self.ax_wf.plot(times, y, linewidth=0.5, label=os.path.basename(path))
    ...
    def _show_midi(self, path):
        ...
        self.ax_wf.set_xlim(*self.full_xlim); self.canvas_wf.draw()
        ...
        self.ax_sp.axis("off"); self.full_xlim_sp=(0,end_t)
        self.canvas_sp.draw()
    ...
    def compare(self):
        sels = self.tree.selection()
        if len(sels)!=2:
            messagebox.showerror("Compare","Select exactly two files to compare.")
            return
        ...
        # overlay waveforms/spectrograms or piano-rolls
        # WAVEFORM
        self.ax_wf.clear()
        for idx,p in enumerate(paths):
            ...
            if ext in AUDIO_EXTS:
                ...
                self.ax_wf.plot(t, y, linewidth=0.8, alpha=0.7,
                                label=label)
            else:
                pm = pretty_midi.PrettyMIDI(p)
                for note in sum((inst.notes for inst in pm.instruments), []):
                    self.ax_wf.hlines(note.pitch + idx*4,
                        note.start, note.end,
                        linewidth=4, alpha=0.7,
                        label=label if note is pm.instruments[0].notes[0] else None)
        self.ax_wf.set_ylabel("Signal / pitch"); self.ax_wf.set_xlabel("Time (s)")
        if hasattr(self,"full_xlim"):
            self.ax_wf.set_xlim(*self.full_xlim)
        self.ax_wf.legend(); self.canvas_wf.draw()

        # SPECTROGRAM or MIDI-notation
        self.ax_sp.clear()
        if all(e in AUDIO_EXTS for e in exts):
            for idx,p in enumerate(paths):
                ...
                ts    = np.arange(Sdb.shape[1])*512/sr
                self.ax_sp.imshow(
                    Sdb if idx==0 else np.roll(Sdb,50,axis=0),
            ...
            # overlay piano-roll lightly
            for idx,p in enumerate(paths):
                pm = pretty_midi.PrettyMIDI(p)
                for note in sum((inst.notes for inst in pm.instruments), []):
                    circ = Circle((note.start,note.pitch), radius=0.15,
                                  alpha=0.7,
                                  facecolor=("blue" if idx==0 else "red"))
    ...
    # ─── PLAYBACK ──────────────────────────────────────────────────────────
    def play(self):
        # single play (music if audio, otherwise midi)
    ...
    def play_both(self):
        # simultaneous for two audio
        if not hasattr(self, "compare_paths"):
            messagebox.showerror("Play Both","First hit Compare on two audio files.")
            return
        p1,p2 = self.compare_paths
        if os.path.splitext(p1)[1] not in AUDIO_EXTS or os.path.splitext(p2)[1] not in AUDIO_EXTS:
            messagebox.showerror("Play Both","Both must be audio files.")
            return
        s1 = mixer.Sound(p1)
        s2 = mixer.Sound(p2)
