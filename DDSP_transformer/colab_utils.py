...
# Assume DEFAULT_SAMPLE_RATE and note_seq.audio_io.wav_data_to_samples_pydub are defined or imported properly
...
def audio_bytes_to_np(wav_data,
    ...
    Args:
      ...
      normalize_db: Normalize the audio to this many decibels. Set to None to skip
        normalization step.
      mono: Force stereo signals to single channel. If false, output can be one or
        two channels depending on the source signal.
...
def upload(filepath, sample_rate=DEFAULT_SAMPLE_RATE, normalize_db=None):
    """Load a collection of audio files (.wav, .mp3) from disk using a local file dialog.
    ...
    Returns:
      A tuple of lists, (filenames, numpy_arrays).
    """
    # Check if 'filepath' is a single string, and if so, wrap it in a list.
    if isinstance(filepath, str):
        fnames = [filepath]
    else:
        fnames = list(filepath)
...
def save_dataset_statistics(data_provider,
  ...
  Args:
    data_provider: A Data/home/alels_star/Desktop/ekpa_diplom/new3/test_GUI/predicted_songx_6573_4a.wavProvider from ddsp.training.data.
