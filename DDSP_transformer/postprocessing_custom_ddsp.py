...
# -*- coding: utf-8 -*-
"""
Created on Tue Apr  8 01:08:02 2025
...
def fit_quantile_transform(loudness_db, mask_on, inv_quantile=None):
  ...
  if inv_quantile is None:
    ...
    #loudness_norm = np.ravel(tf.identity(loudness_db))[:, np.newaxis]
    loudness_norm = np.copy(np.ravel(tf.identity(loudness_db)))[:, np.newaxis]
